RSpec.describe Fountainhead::EventSubscriberAdapters::Postgres do
  let(:event_source) { Fountainhead::EventSourceAdapters::Postgres.new(connection) }
  let(:event_id) { 5 }
  subject(:adapter) { Fountainhead::EventSubscriberAdapters::Postgres.new(connection, event_source) }
