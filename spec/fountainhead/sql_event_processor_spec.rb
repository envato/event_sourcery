RSpec.describe Fountainhead::Projector do
  let(:projector_class) {
    Class.new do
      include Fountainhead::Projector
  ...
  describe '#reset' do
    let(:event) { Fountainhead::Event.new(body: {}, aggregate_id: aggregate_id, type: :terms_accepted, id: 1) }
  ...
  describe '#process' do
    ...
    context 'with more than one table' do
      ...
      it 'throws a DefaultTableError when #table is used' do
        expect {
          projector.process(event)
        }.to raise_error(Fountainhead::TableOwner::DefaultTableError)
      end
    end
  end
end
