RSpec.describe ESFramework::DownstreamEventProcessor do
  let(:dep_class) {
    Class.new do
      include ESFramework::DownstreamEventProcessor
  ...
  let(:dep_name) { 'my_dep' }
  let(:event_source_adapter) { ESFramework::EventSourceAdapters::Memory.new(events) }
  let(:event_source) { ESFramework::EventSource.new(event_source_adapter) }
  ...
  it 'has a default processor_name of the class name' do
    allow(dep_class).to receive(:name).and_return('ESFramework::EventSource')
    expect(dep_class.processor_name).to eq 'ESFramework::EventSource'
  end
  ...
  describe '#process' do
    ...
    context 'with a DEP that emits events' do
      let(:event_1) { ESFramework::Event.new(id: 1, type: 'terms_accepted', aggregate_id: aggregate_id, body: { time: Time.now }) }
      let(:event_2) { ESFramework::Event.new(id: 2, type: 'echo_event', aggregate_id: aggregate_id, body: event_1.body.merge(ESFramework::DownstreamEventProcessor::DRIVEN_BY_EVENT_PAYLOAD_KEY => 1)) }
      let(:event_3) { ESFramework::Event.new(id: 3, type: 'terms_accepted', aggregate_id: aggregate_id, body: { time: Time.now }) }
      let(:event_4) { ESFramework::Event.new(id: 4, type: 'terms_accepted', aggregate_id: aggregate_id, body: { time: Time.now }) }
      let(:event_5) { ESFramework::Event.new(id: 5, type: 'terms_accepted', aggregate_id: aggregate_id, body: { time: Time.now }) }
      let(:event_6) { ESFramework::Event.new(id: 6, type: 'echo_event', aggregate_id: aggregate_id, body: event_3.body.merge(ESFramework::DownstreamEventProcessor::DRIVEN_BY_EVENT_PAYLOAD_KEY => 3)) }
      ...
      xit "emits events when it reaches the end of the stream as captured in setup" do
        ...
        expect(latest_events(2).map(&:body).map{|b| b[ESFramework::DownstreamEventProcessor::DRIVEN_BY_EVENT_PAYLOAD_KEY]}).to eq [3, 4]
        dep.process(event_5)
      ...
      it 'releases the clutch after it has processes the latest event captured in setup, not before' do
        ...
        expect(latest_events(2).map(&:body).map{|b| b[ESFramework::DownstreamEventProcessor::DRIVEN_BY_EVENT_PAYLOAD_KEY]}).to eq [4, 5]
      end
      ...
      context "when the event emitted hasn't been defined in emit_events" do
        ...
        it 'raises an error' do
          expect {
            dep.process(event_1)
          }.to raise_error(ESFramework::DownstreamEventProcessor::UndeclaredEventEmissionError)
        end
      end
