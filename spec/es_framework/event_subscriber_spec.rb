RSpec.describe ESFramework::EventSubscriber do
  let(:events) { [] }
  let(:event_source_adapter) { ESFramework::EventSourceAdapters::Postgres.new(connection) }
  let(:event_source) { ESFramework::EventSource.new(event_source_adapter) }
  let(:event_subscriber_adapter) { ESFramework::EventSubscriberAdapters::Postgres.new(connection, event_source) }
  subject(:event_subscriber) { ESFramework::EventSubscriber.new(event_subscriber_adapter) }
