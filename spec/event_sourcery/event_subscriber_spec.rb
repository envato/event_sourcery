RSpec.describe EventSourcery::EventSubscriber do
  let(:events) { [] }
  ...
  let(:event_subscriber_adapter) { EventSourcery::EventSubscriberAdapters::Postgres.new(connection, event_source) }
  subject(:event_subscriber) { EventSourcery::EventSubscriber.new(event_subscriber_adapter) }
